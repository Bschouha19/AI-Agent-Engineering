...
| Chapter 13 (Agent Security) | Introduces per-instance agent identity (SPIFFE-style, distinct from A2A's Agent Card mechanism) via a real 1.5-million-agent breach case study — a related but separately-scoped identity problem, not a deeper treatment of Agent Card spoofing or replay attacks specifically |
...
**Optional challenge:** Take this chapter's Production Project and imagine the vendor's status-checking agent itself needs to consult a THIRD agent (its own upstream infrastructure provider) before answering. Sketch, at a high level, what changes and what stays the same about the trust-verification chain — does Aperture Cloud need to verify the upstream provider's identity too, or can it reasonably trust the vendor's own verification of that hop? This is a real, current open question in multi-hop agent trust chains — this course doesn't return to it with a formal answer in a later chapter, so treat working through it here as the actual exercise, not a preview of content still to come.
